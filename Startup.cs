...
namespace Assignment2
{
    ...
    public class Startup
    {
        ...
        {
            services.AddControllersWithViews(); // allows us to use MVC
        }
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            {
                // here is where I'll add an error page
            }

            app.UseNodeModules();                                       // bring in NPM for the project
            app.UseStaticFiles();                                       // allow access to html files, img, css, and js files too
            app.UseRouting();                                           // allows us to use routing -- allows dynamic pages for unique users
            app.UseEndpoints(cfg =>                                     // what page we are going to serve -- from routing
            {                                                           // we use routing to find an endpoint
                cfg.MapControllerRoute("Default",                       // find the controller that matches, then the action, 
                    "{controller}/{action}/{id?}",                      // then the ID, and specify a default in case of no match
